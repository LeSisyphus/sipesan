...
@php
    ...
    $nim = $mahasiswa?->nim ?? $user?->nim ?? '-';
    $namaProdi = $prodi?->nama_prodi ?? '-';
    $angkatan = $mahasiswa?->angkatan ?? '-';
    $statusMahasiswa = $mahasiswa?->status ?? 'Aktif';

    $totalPengajuan = $totalPengajuan
        ?? $total
        ?? (($selesai ?? 0) + ($diproses ?? 0) + ($menunggu ?? 0) + ($ditolak ?? 0));

    $tanggalLahir = $mahasiswa?->tanggal_lahir
        ? \Illuminate\Support\Carbon::parse($mahasiswa->tanggal_lahir)->format('Y-m-d')
        : '';
            ...
            >
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input
                            type="text"
                            value="{{ $mahasiswa?->fakultas ?? 'Ilmu Komputer' }}"
                            readonly
                            class="glass-input w-full mt-2 px-5 py-3"
                        >
                    </div>
